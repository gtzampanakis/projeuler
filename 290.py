...
T.update({
		...
		(2, ds(2), 2*M, K_for_mult(2)) : 1,
		(3, ds(3), 3*M, K_for_mult(3)) : 1,
		(4, ds(4), 4*M, K_for_mult(4)) : 1,
		(5, ds(5), 5*M, K_for_mult(5)) : 1,
		(6, ds(6), 6*M, K_for_mult(6)) : 1,
		(7, ds(7), 7*M, K_for_mult(7)) : 1,
		(8, ds(8), 8*M, K_for_mult(8)) : 1,
		(9, ds(9), 9*M, K_for_mult(9)) : 1,
})
...
for psi in xrange(1, MAX_PSI):
	...
	for key, count in T.iteritems():
		...
		left_side = d
		right_side = 11*d - 9*k
		if key[0] != 0 and left_side == right_side:
